 ...
 * /app/[businessSlug]
 *
 * Native-app rendering alias.
 * This is the URL loaded inside the Capacitor WebView.
 * It renders exactly the same content as /book/[slug]?mode=app
 * without requiring ?mode=app in the URL — cleaner for deep links and
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const { data: business } = await supabase
    ...
    .single() as {
      data: Pick<import('@/types/database').Business, 'name' | 'slug' | 'description' | 'app_name' | 'app_icon_url' | 'logo_url'> | null
      error: unknown
    }
  ...
  return {
    ...
    description: business.description ?? `Book at ${business.name}`,
    manifest: `/book/${business.slug}/manifest`,
    appleWebApp: { capable: true, title: appName, statusBarStyle: 'default' },
    icons: iconUrl
      ? { icon: [{ url: iconUrl, sizes: '512x512' }], apple: [{ url: iconUrl, sizes: '512x512' }] }
      : undefined,
  }
}
...
export default async function AppPage({ params }: PageProps) {
  ...
  const [{ data: servicesRaw }, { data: staffRaw }, { data: hoursRaw }, { data: configRaw }] =
    await Promise.all([
      ...
      supabase.from('staff_members').select('*').eq('business_id', business.id).eq('active', true).order('name'),
      supabase.from('working_hours').select('*').eq('business_id', business.id).order('day_of_week'),
  ...
  // Resolve existing booking URL from session cookie
  ...
  if (sessionToken) {
    ...
    if (session) {
      await serviceSupabase.from('customer_sessions').update({ last_seen_at: now.toISOString() }).eq('id', session.id)
  ...
  return (
    <>
    {currentCustomerUser && <TrackVisit slug={businessSlug} />}
    <WebsiteShell
